...
export function generateTrack(seed: number, config: TrackConfig = {}): Track {
    const { controlPoints = 16, minRadius = 300, maxRadius = 520, trackWidth = 90, obstacleCount = 15 } = config;
    ...
    const obstacles = rawObstacles.filter(o => distPointToSegment(o.position, startLine.a, startLine.b) >= START_LINE_CLEARANCE + o.radius);
