...
export function generateObstacles(rand: () => number, centerLine: Point[], trackWidth: number, count: number): Obstacle[] {
    ...
    while (obstacles.length < count && attempts < count * OBSTACLE_ATTEMPTS_FACTOR) {
        ...
        if (obstacles.some(o => dist(o.position, pos) < o.radius + radius + OBSTACLE_MIN_DIST))
            continue;
